...
def h_state(state, _state=None):

    value = 0

    # relatively close to each other
    if (len(state.piece_hexes) > 0):
        piece = state.piece_hexes.copy().pop()
        dist = sum(math.sqrt(math.pow(piece[0] - qr[0], 2) + math.pow(piece[1] - qr[1], 2)) for qr in state.piece_hexes)
    ...
    value += 1.5 *(len(state.piece_hexes) * 7 - h(state))

    # no emnemy around
    for q, r in state.piece_hexes:
        for step_q, step_r in HEX_STEPS:
            for dist in range(1, 2):
                qr_t = q+step_q*dist, r+step_r*dist # qr_t = 'target' hex
                if qr_t in state.board.block_hexes and state.board.exit_dist((q, r)) <= state.board.exit_dist(qr_t):
                    value -= 1/dist
    return value
...
class State:
    ...
    def __hash__(self):
        ...
        """
        return hash(repr(frozenset(self.piece_hexes)))
